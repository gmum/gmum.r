...
test_that('SVM fucntions is fool proof', {
  ...
  expect_error( SVM(f, ds, lib="xyz"), "No such library. Avaiable are: libsvm" )
  expect_error( SVM(f, ds, kernel="xyz"), "No such kernel type. Avaiable are: linear, poly, rbf, sigmoid")
  expect_error( SVM(f, ds, prep="xyz"), "No such preprocess type, Avaiable are: 2e, none" )
  expect_error( SVM(f, ds, C = -1), "Wrong parameters" )
  expect_error( SVM(f, ds, gamma = -1), "Wrong parameters" )
  expect_error( SVM(f, ds, degree = 0), "Wrong parameters" )
