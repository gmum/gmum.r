...
for(i in seq(min_npoints * 2, max_npoints * 2, by=npoints_step * 2))
{
    ...
    t = t+  as.numeric(system.time(c <- cec(centers=3, nstart=1, x=points, centers.init='random',iter.max=cran_max_iters, type='spherical'))[3])
    t = t+as.numeric(system.time(c <- cec(centers=3, nstart=1, x=points, centers.init='random',iter.max=cran_max_iters, type='spherical'))[3])
    result[result_iter, ] = c(t*0.333, c$iterations)
