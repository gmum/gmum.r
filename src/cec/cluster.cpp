...
namespace gmum {
  ...
  arma::rowvec Cluster::initializeMean(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points) {
  ...
  ClusterUseCovMat::ClusterUseCovMat(int _count,
  ...
  ClusterUseCovMat::ClusterUseCovMat(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points): Cluster(id, assignment, points) {
  ...
  arma::mat ClusterUseCovMat::getCovMat(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points) {
    return covMat;
  }
  ...
  ClusterOnlyTrace::ClusterOnlyTrace(
      unsigned int id, const std::vector<unsigned int> & assignment, 
      const arma::mat & points): Cluster(id, assignment, points) {
    initializeMean(id,assignment,points);
  ...
  void ClusterOnlyTrace::computeCovMatTrace(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points) {
    covMatTrace = 0.0;
    for(unsigned int i = 0; i < points.n_rows; i++)
  ...
  boost::shared_ptr<Cluster> ClusterOnlyTrace::addPoint(
    ...
    double ntrace = ((covMatTrace + dot(meanDiff, meanDiff))*count  + dot(r, r))
        /ncount;
  ...
  arma::mat ClusterOnlyTrace::getCovMat(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points) {
  ...
  boost::shared_ptr<ClusterUseCovMat> ClusterStandard::createInstance(
      int _count, const arma::rowvec &_mean, const arma::mat &_covMat) {
    return boost::shared_ptr<ClusterUseCovMat>(new ClusterStandard(_count,
                                                                   _mean,
                                                                   _covMat));
  }

  ClusterStandard::ClusterStandard(int _count, 
                                   const arma::rowvec &_mean,
                                   const arma::mat &_covMat): 
  ...
  ClusterCovMat::ClusterCovMat(const arma::mat & sigma,
                               unsigned int id, 
			       const std::vector<unsigned int> &assignment,
			       const arma::mat &points)
      : ClusterUseCovMat(id, assignment, points) {
  ...
  ClusterCovMat::ClusterCovMat(const arma::mat & _invSigma,
                               double _sigmaDet, int _count,
			       const arma::rowvec & _mean, 
                               const arma::mat & covMat): 
      ClusterUseCovMat(_count, _mean, covMat), invSigma(_invSigma), 
      sigmaDet(_sigmaDet) {
    calculateEntropy();
  }
  ...
  boost::shared_ptr<ClusterUseCovMat> ClusterCovMat::createInstance(
      int _count, const arma::rowvec & _mean, const arma::mat & _covMat) {
    return boost::shared_ptr<ClusterUseCovMat>(new ClusterCovMat(invSigma,
        sigmaDet ,_count,_mean, _covMat));  
  }

  ClusterConstRadius::ClusterConstRadius(
      double r, unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points): 
  ...
  boost::shared_ptr<ClusterOnlyTrace> ClusterConstRadius::createInstance(
    ...
    return boost::shared_ptr<ClusterOnlyTrace>(new ClusterConstRadius(
        r, _count, _mean, _covMatTrace));
  }


  ClusterSpherical::ClusterSpherical(
      unsigned int id, const std::vector<unsigned int> &assignment,
      const arma::mat &points):
  ...
  ClusterSpherical::ClusterSpherical(int _count,
                                     const arma::rowvec &_mean,
                                     double  _covMatTrace):
  ...
  boost::shared_ptr<ClusterOnlyTrace> ClusterSpherical::createInstance(
      int _count, const arma::rowvec & _mean, double _covMatTrace) {
    return boost::shared_ptr<ClusterOnlyTrace>(new ClusterSpherical(
       _count, _mean, _covMatTrace));
  }
  ...
  void ClusterDiagonal::calculateEntropy() {
     _entropy =  N*log(2*M_PI*M_E)/2 + log(arma::det(arma::diagmat(covMat)))/2 ;
  }

  boost::shared_ptr<ClusterUseCovMat> ClusterDiagonal::createInstance(
      int _count, const arma::rowvec & _mean, const arma::mat & _covMat) {
    return boost::shared_ptr<ClusterUseCovMat>(new ClusterDiagonal(_count, _mean, _covMat));
  }
}
