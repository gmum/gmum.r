...
Params processArguments(const Rcpp::List &list) {
	...
	if (list.containsElementNamed(CONST::dataset)) {
		...
		Rcpp::NumericMatrix proxyDataset = Rcpp::as < Rcpp::NumericMatrix
				> (list[CONST::dataset]);
	...
	if (list.containsElementNamed(CONST::logClusters)
			&& Rcpp::as<bool>(list(CONST::logClusters)))
		params.logNrOfClusters = true;
	else
		params.logNrOfClusters = false;

	if (list.containsElementNamed(CONST::energy)
			&& Rcpp::as<bool>(list(CONST::energy)))
		params.logEnergy = true;
	else
		params.logEnergy = false;
	...
	if (list.containsElementNamed(CONST::CLUSTERS::init)) {
		std::string init = Rcpp::as < std::string
				> (list(CONST::CLUSTERS::init));
		if (init.compare(CONST::CLUSTERS::random) == 0)
			...
			if (list.containsElementNamed(CONST::centroidsList)) {
				...
				Rcpp::List desc = Rcpp::as < Rcpp::List
						> (list[CONST::centroidsList]);

				for (Rcpp::List::iterator it = desc.begin(); it != desc.end();
						++it)
					params.centroids.push_back(
							Rcpp::as < std::vector<double> > (*it));
	...
	if (params.clusters.size() > 0)
		...
		if (list.containsElementNamed(CONST::CLUSTERS::type)) {
			std::string type = Rcpp::as < std::string
					> (list[CONST::CLUSTERS::type]);
			//Rcpp::stop(type);
		...
		if (list.containsElementNamed(CONST::CLUSTERS::covMat)) {
			...
			Rcpp::NumericMatrix covMatProxy = Rcpp::as < Rcpp::NumericMatrix
					> (list[CONST::CLUSTERS::covMat]);
		...
		if (list.containsElementNamed(CONST::CLUSTERS::functionName)) {
			...
			params.functionName = Rcpp::as < std::string
					> (list[CONST::CLUSTERS::functionName]);
		} else {
...
CEC *findBestCEC(const Params &params) {
	...
	boost::shared_ptr<Hartigan> hartigan(
			new Hartigan(params.logNrOfClusters, params.logEnergy));

	Assignment *assignmentType;
	switch (params.assignmentType) {
	case krandom:
		assignmentType = new RandomAssignment(*(params.dataset),
				params.nrOfClusters);
		break;
	case kkmeanspp:
		assignmentType = new KmeansppAssignment(*(params.dataset),
				params.nrOfClusters);
		break;
	case kcentroids:
		assignmentType = new CentroidsAssignment(*(params.dataset),
				params.centroids);
		break;
	}
