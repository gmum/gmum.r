...
Hartigan::Hartigan(bool logNrOfClusters, bool logEnergy) :
		Algorithm(logNrOfClusters, logEnergy) {
}
...
double Hartigan::calcWholeEntropy(double crossEntropy, int pointsInCluster,
		int numberOfPoints) {
	double p = 1.0 * pointsInCluster / numberOfPoints;
	return p * (crossEntropy - log(p));
}

double Hartigan::calcEntropyChange(const Cluster &A, const Cluster &B,
		int numberOfPoints) {
...
SingleResult Hartigan::singleLoop(const arma::mat &points,
		std::vector<unsigned int> &assignment, double killThreshold,
		std::vector<boost::shared_ptr<Cluster> > &clusters) {
	...
	for (unsigned int i = 0; i < numberOfPoints; i++) {
		...
		for (unsigned int k = 0; k < clusters.size(); k++)
			if (k != source) {
				...
				boost::shared_ptr<Cluster> oldSource, oldTarget, newSource,
						newTarget;
				...
				try {
					...
					double sourceEntropyChange = calcEntropyChange(*newSource,
							*oldSource, numberOfPoints);
					double targetEntropyChange = calcEntropyChange(*newTarget,
							*oldTarget, numberOfPoints);
				...
				if (wholeEntropyChange < 0) {  //newEntropy < oldEntropy
					...
					try {
						...
						if (clusters[source]->size()
								< std::max(int(killThreshold * numberOfPoints),
										dimension + 1)) {
	...
	for (unsigned int j = 0; j < numberOfPoints; j++) {
		...
		if (assignment[j] == source) {
			...
			for (unsigned int l = 0; l < clusters.size(); l++) {
				...
				boost::shared_ptr<Cluster> newTarget = clusters[l]->addPoint(
						pointToAssign);

				double entropyChange = calcEntropyChange(*newTarget, *oldTarget,
						numberOfPoints);
