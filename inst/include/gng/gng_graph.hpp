...
typename RAMGNGGraph<Node, Edge, Mutex>::EdgeIterator RAMGNGGraph<Node, Edge,
    ...
    this->unlock();
    DBG_PTR(m_logger, 10, "ExtGraphNodeManager()::removeEdge Not found edge!");
    return g[a].end();
...
void RAMGNGGraph<Node, Edge, Mutex>::serialize(std::ostream &output) {
    ...
    DBG_PTR(m_logger, 7, "GNGGraph::Serializing edges");
...
void RAMGNGGraph<Node, Edge, Mutex>::load(std::istream &input) {
    ...
    DBG_PTR(m_logger, 5, "Read in " + to_str(bufor_size) + " sized graph with " +
...
void RAMGNGGraph<Node, Edge, Mutex>::resizeGraph() {
    ...
    DBG_PTR(m_logger, 5, "GNGGraph::resizing");
    ...
    DBG_PTR(m_logger, 5, "GNGGraph::resizing done");
    DBG_PTR(m_logger, 5, to_str(first_free));
    DBG_PTR(m_logger, 5, to_str(next_free[previous_size]));
}
