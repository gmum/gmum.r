...
for (p in seq(0.01, 0.3, 0.01)){
  print(sprintf("iteration %i/%i", as.integer(p/0.01), as.integer(0.3/0.01)))
  ...
  gmum_svm <- SVM(x[train],y[train], core="svmlight", kernel=kernel, C=1, gamma=1)
...
test_lens <- rev(rep(nrow(x), length(lens)) - lens)
